...
_modules["auth"] = function()
    ...
    if key == "michaeldeptrai" then
        Auth.IsAuthenticated = true
        Auth.KeyData = {
            key = key,
            expiresIn = 999999, -- Set thời gian hết hạn cực lớn
            hwidBound = true,
            isFreeKey = false,
            isPremium = true, -- Cho thành Premium luôn cho máu
            discordLinked = true
        }
        -- Lưu key này vào máy để lần sau tự login
        storeKeySplit(key)
        Utils.saveKey(key)
        
        callback(true, "Michael đẹp trai vcl! Đã bypass thành công.", Auth.KeyData)
        return -- Dừng function tại đây, không chạy xuống phần check server nữa
    end
    --------------------------
